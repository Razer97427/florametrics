...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    try {
        if (!empty($new_pass)) {
            ...
            $upd = $conn->prepare("UPDATE florametrics SET nomcomplet = ?, role = ?, password = ?, status = ? WHERE login = ?");
            $upd->bind_param("sssss", $new_nom, $new_role, $hashed_pass, $new_status, $login_to_edit);
        } else {
            $upd = $conn->prepare("UPDATE florametrics SET nomcomplet = ?, role = ?, status = ? WHERE login = ?");
            $upd->bind_param("ssss", $new_nom, $new_role, $new_status, $login_to_edit);
        }

        if ($upd->execute()) {
            // $message = "Utilisateur mis à jour avec succès !";
            $_SESSION['message_success'] = "Utilisateur '" . htmlspecialchars($login_to_edit) . "' modifié avec succès !";
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select name="status" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--border);">
                    <option value="A" <?= ($user['status'] === 'A') ? 'selected' : '' ?>>Actif</option>
                    <option value="N" <?= ($user['status'] === 'N') ? 'selected' : '' ?>>Non Actif</option>
                </select>
            </div>
